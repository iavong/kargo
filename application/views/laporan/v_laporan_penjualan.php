                ...
                <div class="clearfix mb-20">
                    ...
                    <?php foreach ($penjualan->result() as $row) : ?>
                        ...
                        if ($row->jenis_pembayaran == 'cash') {
                            $cash[] = $deleted ? $row->biaya_total : null;
                        }
                        if ($row->jenis_pembayaran == 'deposit') {
                            $deposit[] = $deleted ? $row->biaya_total : null;
                        }
                        if ($row->jenis_pembayaran == 'debit') {
                            $debit[] = $deleted ? $row->biaya_total : null;
                        }
                        
                        $totalBerat += $deleted ? $row->berat : null;
                        $totalPenjualan += $deleted ? $row->biaya_total : null;
                    ...
                    <?php foreach ($penjualanBulan->result() as $row) : ?>
                        ...
                        $totalBeratBulan += $deleted ? $row->berat : null;
                        $totalPenjualanBulan += $deleted ? $row->biaya_total : null;
                        ?>
                    <?php endforeach; ?>
